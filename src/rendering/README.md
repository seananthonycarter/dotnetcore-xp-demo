...
test/preview at [www.dotnetcore_xp_demo.localhost](https://www.dotnetcore_xp_demo.localhost).
Be sure to watch the `rendering` container logs for compilation errors. To debug, you
can attach to the `dotnetcore-xp-demo.exe` process inside the `rendering` container.
